...
const DataTable = <T,>({
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const deletableIds = data
        .filter((row) => !canDelete || canDelete(row))
  ...
  const deletableData = canDelete ? data.filter(canDelete) : data;
  const currentSelections = data?.filter?.((item) =>
    selectedIds?.includes?.(getRowId(item)),
  );
  const allSelected =
    currentSelections?.length === deletableData?.length &&
    deletableData?.length > 0;
  ...
  const showActionsColumn = !!(
    ...
    (renderActions &&
      (actionPermissions.length === 0 || hasAnyPermission(actionPermissions)))
  );

  return (
    <div
      className={`bg-white dark:border-(--card-border-color) dark:bg-(--card-color) rounded-lg border border-gray-200 shadow-sm overflow-hidden transition-all ${tableClassName}`}
    >
      {/* Table */}
            ...
            <tr>
              ...
              {columns.map((column, index) => {
                ...
                return (
                  <th
                    key={index}
                    className={`px-6 py-4 text-left rtl:text-right text-xs font-semibold text-gray-600 dark:text-gray-400 ${column.className || ""} ${columnClassNames[index] || ""} ${column.sortable ? "cursor-pointer select-none hover:text-gray-900 dark:hover:text-white" : ""}`}
                    onClick={() => column.sortable && handleSort(key)}
                  >
                    <div className="flex items-center gap-1 ">
                      <span
                        className={`${isActive ? "" : "capitalize text-[15px] font-bold"}`}
                      >
                        {column.header}
                      </span>
                      {getSortIcon(column)}
              ...
              {showActionsColumn && (
                <th
                  className={`px-6 py-3 text-left rtl:text-right text-[14px] font-semibold text-gray-600  dark:text-gray-400 ${actionClassName}`}
                >
                  Actions
                </th>
              )}
            ...
            {isLoading && data.length === 0
              ? Array.from({ length: limit || 5 }).map((_, rowIndex) => (
                  ...
                  >
                    ...
                    {columns.map((_, colIndex) => (
                      ...
                      >
                        <Skeleton
                          className={`h-4 ${colIndex === 0 ? "w-2/3" : "w-1/2"}`}
                        />
                      </td>
                    ))}
                    {showActionsColumn && (
              ...
              : data.map((row, index) => {
                  ...
                  return (
                    <tr
                      key={index}
                      className={`hover:bg-gray-50 dark:hover:bg-(--table-hover) dark:bg-(--card-color) transition-colors dark:border-(--card-border-color) ${selectedIds.includes(rowId) ? "bg-(--input-color) dark:bg-(--table-hover)" : ""}`}
                    >
                      {hasDeleteActions && (
                        <td className={`px-6 py-4 ${selectionClassName}`}>
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              handleSelectOne(rowId, checked === true)
                            }
                      ...
                      {columns.map((column, colIndex) => {
                        ...
                        const valueToCopy = column.copyField
                          ? String(
                              getNestedValue(row, column.copyField as string) ??
                                "",
                            )
                          : column.accessorKey
                        ...
                        return (
                          <td
                            key={colIndex}
                            className={`px-6 py-4 group/row ${column.className || ""} ${columnClassNames[colIndex] || ""}`}
                          >
                            <div className="flex items-center gap-2 max-w-xs sm:max-w-md">
                              <div className="whitespace-normal overflow-hidden text-sm break-all font-medium">
                                {column.cell
                                  ? column.cell(row)
                                  : column.accessor
                                    ? column.accessor(row)
                              ...
                              {column.copyable && (
                                <Button
                                  onClick={(e) =>
                                    !is_demo_mode && handleCopy(e, valueToCopy)
                                  }
                                  disabled={is_demo_mode}
                                  className={`p-1.5! hover:bg-blue-50! bg-[unset]! h-[unset]! shadow-none dark:hover:bg-blue-900/20 rounded-md transition-all dark:hover:bg-(--table-hover)! text-gray-400 hover:text-primary opacity-0 group-hover/row:opacity-100 shrink-0 ${is_demo_mode ? "cursor-not-allowed" : ""}`}
                                  title={
                                    is_demo_mode
                                      ? "Copying restricted in demo mode"
                                      : "Copy to clipboard"
                                  }
                                >
                                  <Copy size={13} />
                            ...
                            {onDelete && (!canDelete || canDelete(row)) && (
                              <Can permission={deletePermission}>
                                <Button
                                  ...
                                  className="w-10 h-10 border-none text-red-600 hover:text-red-600 dark:text-red-500 hover:bg-red-50 rounded-lg transition-all dark:hover:bg-red-900/20 dark:bg-(--page-body-bg) shadow-xs"
                                  disabled={isLoading}
                                  title="Delete"
                                >
