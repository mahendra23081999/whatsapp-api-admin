      ...
      <div className="flex items-center gap-6 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <Select value={limit.toString()} onValueChange={(value) => onLimitChange(Number(value))} disabled={isLoading}>
            <SelectTrigger size="sm" className="w-18 h-9 text-sm rounded-lg bg-white dark:bg-(--card-color) border-slate-200 dark:border-(--card-border-color) shadow-xs focus:ring-blue-500/10">
            ...
            <SelectContent align="start" className="min-w-20 rounded-lg shadow-xl">
              {limitOptions.map((option) => (
                <SelectItem key={option} value={option.toString()} className="rounded-lg cursor-pointer focus:bg-blue-50 focus:text-blue-700">
                  {option}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="flex items-center gap-1.5">
          {getPageNumbers().map((p, index) =>
            p === "..." ? (
              ...
              <Button key={index} variant={page === p ? "outline" : "ghost"} size="sm" onClick={() => onPageChange(Number(p))} disabled={isLoading} className={cn("h-9 w-9 p-0 rounded-lg text-sm font-medium transition-all", page === p ? "bg-blue-50 dark:bg-(--card-color) text-blue-600 border-blue-500 dark:border-blue-500/50 hover:bg-blue-100 dark:hover:bg-(--dark-sidebar) disabled:opacity-100" : "bg-gray-50 dark:bg-(--card-color) text-slate-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-(--dark-sidebar) hover:text-slate-900 dark:hover:text-gray-200")}>
                {p}
              </Button>
            )
          )}
