...
import { AddLanguagePageProps } from "@/src/types/language";
import {
  ALLOWED_IMAGE_TYPES,
  MAX_IMAGE_SIZE,
  MAX_JSON_SIZE,
} from "@/src/data/language";
...
const AddLanguagePage = ({ id }: AddLanguagePageProps) => {
  ...
  const { data: languageData, isLoading: isLoadingLanguage } =
    useGetLanguageByIdQuery(id || "", { skip: !isEditMode });
  ...
  const [frontJsonFileName, setFrontJsonFileName] = useState<string | null>(
    null,
  );

  const [adminJsonFile, setAdminJsonFile] = useState<File | null>(null);
  const [adminJsonFileName, setAdminJsonFileName] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (isEditMode && languageData?.data) {
      ...
      const isStandard = languages.some((l) => l.code === lang.locale);
      if (isStandard) {
        setSelectedLangCode(lang.locale);
        setCustomLocale("");
      } else {
        setSelectedLangCode("other");
        setCustomLocale(lang.locale);
      }
          ...
          <div className="xl:col-span-4 space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 dark:bg-(--card-color) dark:border-(--card-border-color) sm:p-6 p-4">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2 flex flex-col">
                  ...
                  <Select
                    ...
                    onValueChange={(val) => {
                      ...
                      if (val === "other") {
                        ...
                        const isSystemName =
                          languages.some((l) => l.name === name) || name === "";
                        if (isSystemName && selectedLang) {
                  ...
                  >
                    ...
                    <SelectContent className="max-h-64 dark:bg-(--card-color)">
                      {languages.map((lang) => (
                        <SelectItem
                          className="max-h-64 dark:hover:bg-(--table-hover)"
                          key={lang.code}
                          value={lang.code}
                        >
                          {lang.name} ({lang.code})
                ...
                {/* Custom Locale Code Input (Only visible when "Other" is selected) */}
                {selectedLangCode === "other" && (
                  <div className="space-y-2 flex flex-col transition-all duration-300 ease-in-out animate-in fade-in slide-in-from-top-2">
                    <Label
                      htmlFor="customLocale"
                      className="text-sm font-medium text-gray-900 dark:text-gray-400"
                    >
                    ...
                    <Input
                      id="customLocale"
                      placeholder="e.g. en-US, fr-CA, or custom"
                      value={customLocale}
                      onChange={(e) => {
                      ...
                      className="h-11 bg-(--input-color) dark:bg-page-body dark:border-(--card-border-color) focus:border-(--text-green-primary) focus:ring-(--text-green-primary) rounded-lg border-gray-200"
                      required
                    />
                    <p className="text-xs text-gray-400">
                      Enter the ISO language/locale code (e.g., 'es-ES' or
                      'hi-IN').
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-page-body rounded-lg border border-gray-100 dark:border-none transition-all">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Enable for scripts like Arabic or Hebrew
                    </p>
                  </div>
                  <Switch
                    id="isRtl"
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-page-body rounded-lg border border-gray-100 dark:border-none transition-all">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Make this the primary language for the platform
                    </p>
                  </div>
                  <Switch
                    id="isDefault"
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 dark:bg-(--card-color) dark:border-(--card-border-color) sm:p-6 p-4">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4 flex flex-col">
                  ...
                  <div className="flex items-center gap-6 flex-col sm:flex-row">
                    <div className="shrink-0">
                      {flagPreview ? (
                        <div className="relative w-20 h-20 rounded-xl overflow-hidden border-2 border-(--text-green-primary)/10 dark:border-(--text-green-primary)/20 group shadow-sm transition-all hover:border-(--text-green-primary)">
                          <Image
                            src={flagPreview}
                            alt="Flag"
                        ...
                        <div className="w-20 h-20 rounded-lg border-2 border-dashed border-gray-200 dark:border-(--card-border-color) bg-gray-50 dark:bg-(--card-color) flex flex-col items-center justify-center transition-all hover:border-(--text-green-primary)">
                    ...
                    <div className="flex-1 space-y-2">
                      <Label
                        htmlFor="flag"
                        className="inline-flex items-center gap-2.5 px-5 py-2.5 bg-white dark:bg-page-body dark:border-none dark:text-slate-300 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-(--table-hover) transition-all font-medium text-sm text-gray-700 shadow-sm active:scale-95"
                      >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4 flex flex-col">
                    <Label className="text-sm font-medium text-gray-900 dark:text-gray-400">
                      {setting?.app_name || t("common_app_name")} Front JSON
                    </Label>
                    <div className="relative border-2 border-dashed border-gray-200 dark:border-(--card-border-color) rounded-lg p-6 bg-gray-50/50 dark:bg-(--card-color) hover:bg-gray-100/50 dark:hover:bg-(--table-hover) hover:border-(--text-green-primary) transition-all group overflow-hidden">
                      <Input
                        ...
                        onChange={(e) => handleJsonChange(e, "front")}
                      ...
                      <div className="flex flex-col items-center justify-center text-center">
                        ...
                        {frontJsonFileName ? (
                          <div className="space-y-1">
                            <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate max-w-37.5">
                              {frontJsonFileName}
                            </p>
                    ...
                    <a
                      href={getUrlWithBasePath("/assets/files/Front.json")}
                      download="Front.json"
                      className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors w-fit"
                    >
                  ...
                  <div className="space-y-4 flex flex-col">
                    <Label className="text-sm font-medium text-gray-900 dark:text-gray-400">
                      {setting?.app_name || t("common_app_name")} Admin JSON
                    </Label>
                    <div className="relative border-2 border-dashed border-gray-200 dark:border-(--card-border-color) rounded-lg p-6 bg-gray-50/50 dark:bg-(--card-color) hover:bg-gray-100/50 dark:hover:bg-(--table-hover) hover:border-(--text-green-primary) transition-all group overflow-hidden">
                      <Input
                        id="admin_json"
                        type="file"
                        accept=".json,application/json"
                        onChange={(e) => handleJsonChange(e, "admin")}
                        className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                      />
                      <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <a
                      href={getUrlWithBasePath("/assets/files/Admin.json")}
                      download="Admin.json"
                      className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors w-fit"
                    >
                      <Download className="w-3 h-3" />
                  ...
                  <div className="space-y-4 flex flex-col">
                    <Label className="text-sm font-medium text-gray-900 dark:text-gray-400">
                      {setting?.app_name || t("common_app_name")} App JSON
                    </Label>
                    <div className="relative border-2 border-dashed border-gray-200 dark:border-(--card-border-color) rounded-lg p-6 bg-gray-50/50 dark:bg-(--card-color) hover:bg-gray-100/50 dark:hover:bg-(--table-hover) hover:border-(--text-green-primary) transition-all group overflow-hidden">
                      <Input
                        id="app_json"
                        type="file"
                        accept=".json,application/json"
                        onChange={(e) => handleJsonChange(e, "app")}
                        className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                      />
                      <div className="flex flex-col items-center justify-center text-center">
                    ...
                    <a
                      href={getUrlWithBasePath("/assets/files/App.json")}
                      download="App.json"
                      className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors w-fit"
                    >
                      <Download className="w-3 h-3" />
                  ...
                  !appJsonFile && (
                    <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-900/20">
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-400">
                        Existing translation files remain active unless you
                        upload new ones.
                      </p>
                    </div>
                  )}
