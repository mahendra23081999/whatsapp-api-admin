...
const TaxList = ({ taxes, isLoading, total, page, totalPages, onPageChange, limit, onLimitChange, onDelete, onBulkDelete, onSelectionChange, selectedIds, onSortChange, visibleColumns, searchTerm }: TaxListProps) => {
  ...
  const allColumns: (ColumnDef<Tax> & { id: string })[] = [
    ...
    },
    {
      ...
      sortable: true,
      sortKey: "is_active",
      cell: (row) => <Badge className={row.is_active ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 hover:bg-emerald-100" : "bg-gray-100 text-gray-700 dark:bg-page-body dark:text-gray-500 "}>{row.is_active ? t("common_active") : t("common_inactive")}</Badge>,
    },
  ];
