...
const AdminTemplateContainer = () => {
  ...
  const [columnsVisibility, setColumnsVisibility] = useState([
    ...
    { id: "platform", label: "Channel", isVisible: true },
  ...
  const allColumns = [
    {
      ...
      className: "[@media(max-width:1920px)]:min-w-[300px]",
      sortable: true,
      ...
      accessor: (template: AdminTemplate) => (
        ...
        >
          <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center text-primary shrink-0">
    ...
    },
    {
      ...
      accessor: (template: AdminTemplate) => (
        <Badge
          className={`text-[11px] font-semibold capitalize rounded-md px-2 py-0.5 border-0 ${CATEGORY_COLORS[template.category] ?? ""}`}
        >
          {template.category}
        </Badge>
      ),
    },
    {
      ...
      sortKey: "platform",
      accessor: (template: AdminTemplate) => {
        const plat = template.platform || "whatsapp";
        return (
          <Badge
            className={`text-[11px] font-semibold capitalize rounded-md px-2 py-0.5 border-0 ${PLATFORM_COLORS[plat] ?? "bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-400"}`}
          >
            {plat}
          </Badge>
        );
    ...
    <div>
      <CommonHeader
        ...
        searchActions={
          ...
          >
            <SelectTrigger className="w-[180px] h-9 py-5.5 border-slate-200 dark:border-none rounded-lg bg-(--input-color) dark:bg-page-body transition-all shrink-0">
