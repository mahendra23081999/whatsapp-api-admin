    ...
    <div className="bg-white dark:bg-(--card-color) p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-(--card-border-color) shadow-sm space-y-6 sm:space-y-8">
      ...
      <div className="space-y-4 border-t border-slate-100 dark:border-(--card-border-color) pt-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Button type="button" onClick={onAddCard} disabled={cards.length >= 10} className="w-full sm:w-auto h-11 flex items-center justify-center gap-2 px-6 rounded-lg bg-primary hover:bg-blue-700 disabled:opacity-40 text-white text-xs font-bold transition-all">
          ...
          {cards.map((card, idx) => (
            <div key={card.id} className="p-5 bg-slate-50/50 dark:bg-(--dark-body) border border-slate-100 dark:border-(--card-border-color) rounded-xl space-y-5 relative group animate-in zoom-in-95 duration-200">
              ...
              <div className="space-y-2 flex flex-col">
                ...
                />
                {card.file ? (
                  <div className="flex items-center justify-between p-3 bg-blue-50/50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/20 rounded-lg">
                    <div className="flex items-center gap-3">
                  ...
                  <div onClick={() => fileRefs.current[card.id]?.click()} className="border-2 border-dashed border-slate-200 dark:border-(--card-border-color) rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer hover:border-primary hover:bg-blue-50/20 dark:hover:bg-(--table-hover) transition-all">
              ...
              <div className="space-y-2">
                ...
                <CharacterCountWrapper current={card.body_text?.length || 0} max={200}>
                  <Textarea
                    ...
                    className="min-h-20 border-slate-200 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-blue-500/50 transition-all font-medium resize-none"
                  />
                      ...
                      return (
                        <div key={tmpl.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                          <div className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-page-body border border-slate-200 dark:border-(--card-border-color)">
                            {tmpl.type === "url" ? <Link size={12} className="text-sky-500" /> : <MessageSquareReply size={12} className="text-blue-500" />}
