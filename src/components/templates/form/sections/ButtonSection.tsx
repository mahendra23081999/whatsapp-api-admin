    ...
    <div className="bg-white dark:bg-(--card-color) p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-(--card-border-color) shadow-sm space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3">
        {interactiveActions
          ...
          .map((action) => (
            <Button key={action.value} type="button" onClick={() => setInteractiveType(action.value)} className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg border transition-all text-[10px] sm:text-xs font-bold uppercase tracking-wider ${interactiveType === action.value ? "border-primary bg-blue-50/50 hover:bg-blue-50/50 text-blue-700 dark:bg-(--card-color) dark:hover:bg-(--card-color) dark:text-primary shadow-sm" : "border-slate-50 dark:border-(--card-border-color) bg-slate-50/20 hover:bg-slate-50/20 dark:bg-(--dark-sidebar) dark:hover:bg-(--dark-sidebar) dark:hover:border-(--card-border-color) text-slate-400 hover:border-slate-200"}`}>
              {t(action.label)}
            </Button>
          ))}
      </div>
      ...
      <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="space-y-2 flex flex-col">
                ...
                <CharacterCountWrapper current={btn.text?.length || 0} max={60}>
                  <Input
                    ...
                    className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-blue-500/50 transition-all font-medium"
                  />
              ...
              {(btn.type === "url" || btn.type === "website") && (
                <div className="space-y-2 flex flex-col">
                  ...
                  <Input placeholder="https://example.com" value={btn.url || btn.website_url || ""} onChange={(e) => updateButton(btn.id, { url: e.target.value, website_url: e.target.value })} className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-blue-500/50 transition-all font-medium" />
                </div>
              )}
              {btn.type === "phone_call" && (
                <div className="space-y-2 flex flex-col">
                  <Label className="text-xs font-bold text-slate-600 dark:text-gray-400">{t("templates_library_button_phone_label")}</Label>
                  <Input type="number" placeholder="+1 234 567 890" value={btn.phone_number || ""} onChange={(e) => updateButton(btn.id, { phone_number: e.target.value })} className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-blue-500/50 transition-all font-medium" />
                </div>
              )}
            </div>
          </div>
        ...
        {interactiveType === "cta" && (
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {isRestricted && (
              <Button type="button" onClick={() => addButton("copy_code")} className="flex-1 h-11 bg-[unset]! sm:h-12 flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 dark:hover:border-(--card-border-color) dark:text-amber-50 dark:border-(--card-border-color) text-slate-500 font-bold hover:border-blue-500/50 hover:bg-blue-50/30 dark:hover:bg-(--table-hover)! transition-all text-xs sm:text-sm">
